...
# ------------------ Backend forecast functions ------------------
def feet_to_m(ft): return ft * 0.3048
def mm_to_inches(x): return x / 25.4
...
def fetch_open_meteo(model_name):
    ...
    df = df[(df['time_utc'] >= START_UTC) & (df['time_utc'] <= END_UTC)].reset_index(drop=True)
    return df, model_elev

# Fetch models and build ensemble
model_frames = []
model_elevs = {}
for m in models_selected:
    try:
        df, elev = fetch_open_meteo(m)
